...
describe("mapEditorModel", () => {
  it("creates a map draft with explicit gameplay tiles and radar rows", () => {
    ...
    expect(draft.radarPath).toBe("content/maps/ascii/test-map-radar.json");
    ...
    expect(draft.radar.trace.jsonLine).toContain("content/maps/ascii/test-map-radar.json");
  });
  ...
  it("normalizes existing map content that has no radar field", () => {
    ...
    expect(state.draft.radarPath).toBe("content/maps/ascii/test-map-radar.json");

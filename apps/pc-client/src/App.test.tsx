...
describe("App", () => {
  ...
  it("keeps items and mainline event programs wired through content data", () => {
    ...
    expect(eventProgramDefinitions.map((definition) => definition.id)).toEqual(
      expect.arrayContaining([
        "mainline_crash_site_survey_chain",
        "mainline_village_trade_thermal_gear",
        "mainline_hive_entrance_and_hatchery",
        "mainline_warp_pod_final_sequence",
      ]),
    );
    expect(eventProgramDefinitions.every((definition) => definition.domain.startsWith("mainline_"))).toBe(true);
  });

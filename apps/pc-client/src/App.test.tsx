...
describe("App", () => {
  beforeEach(() => {
    ...
    vi.spyOn(Math, "random").mockImplementation(() => {
      randomTick = (randomTick + 1) % 997;
      return 0.5 + randomTick / 1994;
    });
  });
  ...
  it("does not trigger idle chatter for runtime event calls", () => {
    ...
    const callCountBeforeConnect = randomSpy.mock.calls.length;
    fireEvent.click(within(getMikeCrewCard()).getByRole("button", { name: "接通" }));

    expect(randomSpy.mock.calls.length).toBe(callCountBeforeConnect);
    expect(getIdleChatterFlagKeys(readSavedState())).toEqual([]);
  });

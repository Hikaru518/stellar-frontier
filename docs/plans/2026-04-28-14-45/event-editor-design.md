...
- **Game Editor**：独立于玩家游戏入口的内容生产工具入口，位于项目根目录 `editor/`。本轮只开放 Event Editor，并为未来 Character / Map / Item / NPC Editor 保留导航位置。
...
1. 策划或开发者打开根目录 `editor/` 下的 Game Editor，进入轻量 shell。
...
- **入口规则**：editor 位于项目根目录 `editor/`，不挂载玩家游戏 `App`，但可以复用事件类型、校验、渲染和样式 token。
...
- **依赖于**：`content/events/` 的结构化事件文件、`content/schemas/events/` 的 JSON Schema、`handler_registry.json`、`scripts/validate-content.mjs`、`src/events/validation.ts`、`src/events/callRenderer.ts`。
...
- 建立根目录 `editor/` 下的轻量 Game Editor shell，并只开放 Event Editor。

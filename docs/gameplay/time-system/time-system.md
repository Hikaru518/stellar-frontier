...
flowchart TD
  ...
  Time["时间系统：elapsedGameSeconds，timeScale = 8"]
  Crew["队员行动系统：移动 / 待命 / 停止 / 调查"]
  Comms["通讯台系统：队员状态 / 来电 / 剩余时间"]
  Call["通话系统：玩家决策 / 紧急倒计时"]
  Map["地图系统：位置 / 地块 / 调查状态"]
  Resource["资源系统：事件结算 / 库存变化"]
...
| `timeScale` | 数值 | 时间倍率。正式默认值为 `8`；Debug toolbox 仅在开发 / 验收时可临时切换。 |
...
| --- | --- | --- |
| `crewId` | 字符串 | 队员 ID。 |
| `actionType` | 枚举 | 基础行动类型为 `move`、`standby`、`stop`、`survey`；剧情动作由结构化事件或目标定义。 |
...
地图尺寸、合法边界和玩家显示坐标由地图系统配置决定，默认地图为 `8 x 8`。时间系统只关心行动进入下一个目标地块时需要消耗多少游戏秒，不直接假设固定行列范围。

玩家可见坐标以坠毁地点 / 玩家开始地点为 `(0,0)`，可出现负数；内部 tile id 与显示坐标的转换由地图系统维护。
...
采样、修复、交易、使用道具、撤离等剧情动作不属于通用基础行动。它们需要消耗时间时，由结构化事件、`action_request` 或 objective 在内容中声明耗时、消耗、成功条件和失败后果。
...
Garry 正在调查矿床异常，剩余 60 秒。
玩家在通话页面停留 60 秒后仍未下达新指令。
第 1 日 00:06:00，Garry 的调查行动完成，事件系统检查地点反馈。
```
...
| 4 | 调查完成 | 更新地图信息并检查调查事件。 |
| 5 | 剧情动作完成 | 按结构化事件或目标规则结算结果。 |
...
```text
当前时间：第 1 日 00:10:00
Garry 正在调查矿床异常，剩余 120 秒。
玩家停留在控制中心 120 秒，没有进行任何操作。
当前时间变为第 1 日 00:12:00。
Garry 完成调查，系统检查地点事件。
```

结构化事件或目标行动在游戏运行中按到期时间结算。关闭游戏后，这些计时不会继续推进，也不会产生离线产出。
...
- 队员行动剩余时间。
- 地块调查状态。
- 地块发现状态（当前不用于裁剪 PC 地图可见范围或移动候选目标）。
- 地块调查状态与事件标记。
- 地块危险是否正在倒计时。
...
资源变化由结构化事件或目标结算产生。玩家点击基础行动时不直接获得资源；如果某个地点事件提供取样、回收或交易选项，资源 ID、数量、消耗和失败后果由该事件定义。
...
Garry 正在调查矿床异常，剩余 120 秒。
玩家不操作，停留在控制中心。
第 1 日 00:02:00，Garry 完成调查，系统检查地点事件。
全局 Header 持续显示最新时间。
```
...
Garry 同时正在处理地点事件，剩余 30 秒。
玩家在 Amy 通话页面阅读台词。
第 1 日 00:10:30，Garry 的地点事件处理完成。
通讯台和地图状态更新。
...
Garry 仍在调查矿床异常，仍剩余 120 秒。
玩家进入游戏后，时间继续按 1 现实秒 = 8 游戏秒推进。
```

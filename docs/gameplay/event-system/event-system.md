...
当前 runtime content 只加载回家主线。`content/events/manifest.json` 注册 `mainline_crash_site`、`mainline_resources`、`mainline_village`、`mainline_medical`、`mainline_hive`、`mainline_ending`；旧 `crash_site`、`desert`、`forest`、`mine`、`mountain` 演示事件不再作为正式内容存在。
...
- **高光时刻**：队员调查坠毁点后，事件写入维修日志、雷达线索、玩家日志和后续主线 flag；玩家感觉地图正在记住他的行动。
- **压力时刻**：队员推进巢穴入口后，事件根据诱饵、风险和燃料需求给出专属选项，玩家必须决定是否继续深入。
- **调度时刻**：主线资源链需要稀有矿石、黑曜石和修复套件，玩家安排队员前往丘陵、火山和熔炉地点推进 parent event。
- **长期后果时刻**：玩家完成语言、救援、燃料、坐标和修复链后，`return_home_completed` 改变终局状态并进入返航结算。
...
当前正式事件定义和通话模板只保留主线 JSON。测试可以用 fixture、inline sample 或 sample context 验证事件引擎、条件、效果、graph runner 和 handler 能力；这些测试材料不是 runtime content，不能放入 manifest，也不能依赖 `content/events/definitions` 下已删除的旧演示文件。
...
- **当前内容边界校验**：manifest 只能注册主线 domain；未注册 definitions、call templates 或 presets 会失败；map-object `actions[].event_id` 必须指向存在的事件；默认地图不得引用缺失对象或旧演示对象。
...
- **S1：坠毁点起点**：队员调查当前区域后触发坠毁点主线，维修日志、基础雷达和折跃仓线索写入 world flags、event_log 和后续候选条件。
- **S2：资源链推进**：丘陵矿床、稀有矿石夹层、火山高温采集点和损坏熔炉共同支撑修复套件链路；事件用物品、地图对象和 world history 记录进度。
- **S3：村落与语言 / 交易**：罗塞塔装置、外星村落、商人和村民线索通过主线事件连接语言、交易和医疗救援。
- **S4：医疗与巢穴燃料**：医疗舱、医疗文档、受伤村民营地、湿地诱饵来源和巢穴对象形成救援与燃料链。
- **S5：返航终局**：修复套件、燃料和坐标满足后，折跃仓事件推进 `return_home_completed` 并进入 `EndingPage`。

#### 后续普通事件池方向

普通事件池仍是后续扩展，不属于当前 runtime content。未来如果要恢复资源、见闻、危险提示和地块变化类事件，需要先写入新的 design，更新 manifest 白名单、map-object 边界和 validator，再把内容放入 `content/`。
...
- **项目内策划案**：`docs/plans/2026-04-27-15-33/event-program-model-player-journey-design.md`。借鉴点：全局资产库、事件图运行时、结构化 condition/effect、生产级校验和模型覆盖矩阵。
- **现有实现**：`content/events/definitions/*.json`、`content/events/call_templates/*.json`、`src/events/*`、`src/pages/CallPage.tsx`。借鉴点：已有结构化触发、紧急来电、通话选择和效果结算路径。

## 10. Open Questions

- **Q1：后续普通事件池如何重新进入 runtime？** 当前 runtime 只保留回家主线；后续资源、见闻或危险事件需要先更新 design、manifest 白名单和 validator。
...
| 2026-04-30 | docs/plans/2026-04-30-00-15/minimal-use-case-content-boundary-design.md | 同步当前 runtime content 边界：只加载回家主线，旧演示事件不再作为正式内容；测试 fixture 与 runtime content 分离。 |

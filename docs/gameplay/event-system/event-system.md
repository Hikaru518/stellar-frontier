...
1. 玩家通过通讯台或通话给队员下达移动、调查、采集、建设、撤离或事件处理指令。
...
- **依赖于**：时间系统、队员系统、队员行动状态、地图/地块状态、背包/资源系统、通话模板、内容资产库和 schema 校验。
- **被依赖于**：通讯台、通话页面、地图标记、系统日志、队员日记、后续事件解锁和未来事件编辑器。
...
- **Q1：第一批 5 个样例事件的具体题材与文本如何展开？** 当前已固定覆盖能力，具体剧情文本可以在实施或内容生产任务中展开。
...
- **Q4：进入可玩版本前是否需要迁移或提示策略？** 研发期允许旧存档硬失败；可玩版本前需要重新评估版本升级体验。

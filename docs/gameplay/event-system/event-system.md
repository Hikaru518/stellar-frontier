...
事件系统不承载所有普通玩家指令。通讯台 / 通话层负责创建移动、调查、待命和对象交互等真实队员行动；事件系统消费 `arrival`、`action_complete`、`idle_time` 和 `call_choice` 等事实信号，并把世界反馈表现为日志、runtime call、objective、行动请求或状态变化。

## 2. 设计意图（Design Intent）

事件系统的核心体验是“世界会对玩家派出去的人产生具体回应”。玩家派队员抵达、调查、采集、建设、待命或处理通话后，事件把环境、人物、风险和资源变化反馈给玩家，让地图探索不只是资源按钮。
...
1. 玩家通过通讯台或通话给队员下达移动、调查、采集、建设或事件处理指令；撤离等特殊处理可以由事件选项桥接为真实行动。
...
- **调度时刻**：Lin Xia 发现火山灰线索后创建 objective，玩家可以派 Kael 或其他符合条件的队员去完成采样并推进 parent event。
- **长期后果时刻**：Kael 发现旧世界遗物后，最终选择改变队员标签、地块对象、世界 flag、后续事件池和个人日记。
...
| 调查地块对象 | `120 秒` | 完成后以对象 ID 和对象 tags 进入 trigger payload。 |
| 扫描地块对象 | `90 秒` | 完成后发出 `action_complete`，由事件定义决定是否生成反馈。 |
...
- **S4：跨队员 objective**：Lin Xia 发现火山灰线索后创建目标，Kael 或其他符合条件的队员完成行动后触发 `objective_completed` 并推进 parent event。
- **S5：长期角色和世界后果**：Kael 发现旧世界遗物后，最终结果改变 personality/expertise tag、地块对象、world flag、后续事件池和日记。
...
- **现有原型**：`content/events/events.json`、`src/eventSystem.ts`、`src/pages/CallPage.tsx`。借鉴点：已有触发、紧急来电、通话选择和效果结算路径；这些路径作为重建输入，而不是兼容约束。
...
| 2026-04-28 | docs/plans/2026-04-27-22-56/communication-table-gameplay-design.md | 补充通讯台行动与事件系统边界，按当前实现更新调查 / 扫描触发耗时，并记录普通来电保留与行动桥接冲突两个未决问题。 |

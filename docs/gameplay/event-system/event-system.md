...
- **事件节点（event_node）**：事件图中的执行单元，类型包括 `call`、`wait`、`check`、`skill_check`、`random`、`action_request`、`objective`、`spawn_event`、`log_only`、`end`。
- **运行时事件（runtime event）**：某个事件定义在一局游戏中的实例，保存当前节点、相关队员/地块、选择、随机结果、检定结果、目标、唤醒时间和最终结果。
...
6. `wait`、`check`、`skill_check`、`random`、`action_request`、`objective` 等节点把事件推进给时间系统、条件分支、玩家可见属性检定、随机分支、队员行动或独立目标。
...
- **压力时刻**：Amy 调查带有野兽踪迹的森林地块后触发紧急来电，玩家必须在时间压力和信息不足下选择撤离、隐藏或冒险处理。
...
| `check` | 执行确定性条件分支。 |
| `skill_check` | 执行玩家可见的 `d20 + 队员属性 >= DC` 检定，保存结果并进入成功或失败节点。 |
...
`skill_check` 是一等事件节点，用于玩家已经选择某个带检定意图的通话选项后结算。它读取主队员的属性值，使用确定性 `stableRandom(seed)` 投掷 `d20`，计算 `roll + modifier >= dc`，并把 `roll`、`modifier`、`total`、`dc`、`outcome`、`seed` 和下游节点写入 `runtime event.check_results[store_result_as]`。

可见检定的 UI 规则：

- `call.options[].display_tag` 只控制选项前缀，例如 `[感知]暂不回应...`，不参与逻辑推进。
- `call.options[].check_preview` 保存属性、属性显示名、DC 和骰面，用于 UI 预览；当前只展示标签。
- 玩家点击选项后仍只提交稳定 `option_id`。
- 检定后的下一段 runtime call 必须在 `LIVE TRANSCRIPT` 第一行插入结果：`{队员名称} 骰出了 {x}，加上 {属性} 数值 {y}，最终结果是 {x + y}. 判定要求是 {z}. 检定成功/失败。`
- 结果行中只有 d20 投掷值 `x` 做随机数字闪烁后定格；属性值、总和和 DC 正常逐字播放。测试或 e2e 可以用 `stellar-frontier-e2e-disable-animation=1` 关闭动画。
...
- **Skill check 校验**：`attribute`、`attribute_label`、`dc`、`die_sides`、`store_result_as` 必须合法，`success_node_id`、`failure_node_id` 和成功/失败 effect refs 必须可解析。
...
| 可见属性检定骰面 | `die_sides = 20` |
| 可见属性检定成功条件 | `roll + modifier >= dc` |
...
- **S6：玩家可见检定与后续会谈**：拾荒营地哨线通话中，偷听、强硬和闲聊选项显示属性标签；偷听 / 闲聊是一次性信息检定，失败回到问话并隐藏已尝试选项，成功可设置村长对话入口 flags；后续 `spawn_event` 进入统一村长会谈，并按 world flags 分流合作要求。下一段 `LIVE TRANSCRIPT` 第一行显示 d20 结果。
...
| 2026-05-16 | docs/plans/2026-05-16-14-41/player-visible-skill-check-design.md | 新增玩家可见 d20 `skill_check` 节点、选项展示标签、检定结果 transcript 规则和 d20 数字动画规则。 |

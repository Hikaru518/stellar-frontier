...
当前原型聚焦于五个核心模块：控制中心、通讯台、通话决策、4x4 星球地图，以及人物详情。世界以全局游戏时间持续运转，玩家通过通话向队员下达移动 / 调查 / 撤离等指令；事件、队员、物品等内容数据从 `content/` 下的 JSON 文件加载。
...
- **地图**：以 4x4 网格展示地形、自然资源、建筑、仪器、危险与队员位置。地图只读，不直接下达指令。
...
│   ├── crew/crew.json       # 队员定义
│   ├── events/events.json   # 事件定义
│   ├── items/items.json     # 物品定义
│   └── schemas/             # 上述三类内容的 JSON Schema
...
所有设计文档位于 `docs/`，按"全量 wiki + 增量提案"的方式维护，详见仓库根目录的 [`AGENTS.md`](AGENTS.md)。常用入口：
...
- 子系统 wiki：[队员系统](docs/gameplay/crew/crew.md) · [事件系统](docs/gameplay/event-system/event-system.md) · [时间系统](docs/gameplay/time-system/time-system.md)
...
- 队员、事件、物品的内容数据位于 `content/*.json`，并在 `src/content/contentData.ts` 中加载；改动后请运行 `npm run validate:content`。

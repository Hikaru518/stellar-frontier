...
开发服务器会保留验收与调试 UI，例如右下角 `[DEBUG]`、顶部 `PERF` 性能入口，以及地图页 `MAP TRACE` / 图层开关。
...
正式玩家版会隐藏开发 UI：不显示右下角 `[DEBUG]`、地图页图层开关和地图调试图层；顶部 `PERF` 性能入口会保留。地图只显示渲染层、遮罩层和队员层；`MAP TRACE` 仅保留玩家点击与标记区块产生的操作日志。
...
- **地图**：以 content-authored `256 x 256` explicit tiles 作为玩法事实源，并用 JSON 驱动的 console radar 表现层展示地形、区域、遮罩、队员位置和路线选择。地图只读，不直接下达指令；从通话进入时只标记候选目的地，最终仍需回到通话确认。临时战争迷雾 / `3 x 3` 探索限制已取消，后续会单独设计。
...
- 使用 `npm run dev:pc` 开发时会显示调试 UI；使用 `npm run production` 启动正式玩家版时会隐藏调试 UI，地图只保留渲染层、遮罩层和队员层，`MAP TRACE` 只保留玩家操作日志。

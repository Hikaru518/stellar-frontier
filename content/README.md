...
- `maps/ascii/*.json`: radar presentation files referenced by maps, including glyph rows, tone rows, palette, symbols, trace copy, and regions.
...
- Map radar rows live in `maps/ascii/*.json`; every `glyphRows` / `toneRows` row must match the configured radar world width, and every tone must exist in `palette`.

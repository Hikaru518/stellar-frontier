...
- Replaced scaffold/raw WebSocket business messaging with real Yuan `Terminal` integration on PC and mobile: both use `enable_WebRTC: true`, provide dual-device services, and send typed events via `requestByMessage`. Added Terminal lease/cache for React dev StrictMode and unique per-pairing PC terminal IDs. Local real-Yuan smoke passed for Host + PC + phone WSS message delivery; DataChannel tunnel assertion remains follow-up.
- PR #16 CI initially failed because `rush build` treats Vite warnings as a non-zero build. Added browser stubs/aliases for Yuan's node-only fallback imports and reran local lint/test/build/E2E successfully.
